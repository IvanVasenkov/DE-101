...
CREATE TABLE IF NOT EXISTS geology_dim
(
...
(
 ...
 CONSTRAINT FK_39 FOREIGN KEY ( product_id ) REFERENCES products_dim ( product_id ),
 CONSTRAINT FK_42 FOREIGN KEY ( geo_id ) REFERENCES geology_dim ( geo_id ),
 CONSTRAINT FK_45 FOREIGN KEY ( ship_id ) REFERENCES shipping_dim ( ship_id ),
 CONSTRAINT FK_48 FOREIGN KEY ( reg_id ) REFERENCES region_dim ( reg_id ),
 CONSTRAINT FK_56 FOREIGN KEY ( customer_id ) REFERENCES customers_dim ( customer_id )
);

CREATE INDEX fkIdx_40 ON sales_fact
(
 product_id
);
...
CREATE INDEX fkIdx_46 ON sales_fact
(
 ship_id
);

CREATE INDEX fkIdx_49 ON sales_fact
(
 reg_id
);
